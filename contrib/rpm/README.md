...
The RPM spec file provided here is for Dealtoken-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://Dealtoken.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Dealtoken git repository](https://github.com/Dealtoken/Dealtoken) but are not part of
...
Currently only `Source10` is used, for the example `Dealtoken.conf` file.
...
in the Dealtoken git repository. At present only an SVG version of the Dealtoken
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the Dealtoken
...
needed when 0.12.1 is released, a proper fix is already in the Dealtoken git
master branch.
...
idea to build Dealtoken Core against a static version of that library compiled
...
The `Dealtoken.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `Dealtoken-server` package. At this point, most RPM based
...
The `Dealtoken-server` package should have SELinux support. How to properly do
...
Dealtoken-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/Dealtoken/). He has been
packaging Dealtoken for Fedora at least since 2012.
...
4. I renamed the `Dealtoken` package that contains the Qt GUI to `Dealtoken-core` as
that appears to be how the general population refers to it, in contrast to
`Dealtoken-xt` or `Dealtoken-classic`. I wanted to make sure the general population
