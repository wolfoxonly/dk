...
Also test that nodes that send unsupported service bits to Dealtokend are disconnected
...
class CNodeNoVersionBan(CLazyNode):
    ...
    # NOTE: implementation-specific check here. Remove if Dealtokend ban behavior changes
    def on_open(self):
        super().on_open()
...
class P2PLeakTest(DealtokenTestFramework):
