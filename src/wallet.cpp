...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // Important: versions of Dealtoken before 0.8.6 had a bug that inserted
...
int64 CWallet::GetBalance() const
{
    ...
    {
        LOCK(cs_wallet);
         printf("GetBalance() walllet txCount=%ld\n", mapWallet.size());
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            const CWalletTx* pcoin = &(*it).second;
           printf("GetBalance() GetAvailableCredit=%ld\n", pcoin->GetAvailableCredit() );
...
*/
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // ppcoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
                        ...
                        // change transaction isn't always pay-to-Dealtoken-address
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        {
            printf("CreateCoinStake header.GetBlockTime()==%ld\n",header.GetBlockTime() );
            printf(" CreateCoinStake nStakeMinAge==%d\n",nStakeMinAge);
            printf(" CreateCoinStake txNew.nTime - nMaxStakeSearchInterval==%d\n",txNew.nTime - nMaxStakeSearchInterval);
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64)nMaxStakeSearchInterval) && !fKernelFound; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, header, postx.nTxOffset + sizeof(CBlockHeader), *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake,true))
            {
    ...
    {
        printf("Calculate coin age reward nCredit=:%ld\n",nCredit);
        uint64 nCoinAge;
        ...
        printf("nCoinAge=:%ld\n",nCoinAge);
        int64 nReward = GetProofOfStakeReward(nCoinAge,GetBalance());
        // Refuse to create mint that has zero or negative reward
        printf("Calculate coin age reward nReward=%ld\n",nReward);
        if(nReward <= 0) {
          return false;
        }
        nCredit += nReward;
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CDealtokenAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CDealtokenAddress(address).ToString());
}
