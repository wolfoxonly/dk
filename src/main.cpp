...
// static CBigNum bnProofOfWorkLimit(~uint256(0) >> 32);<zxb>
// static CBigNum bnInitialHashTarget(~uint256(0) >> 40);
// unsigned int nbits = 0x1e0fffff;
// static CBigNum bnProofOfWorkLimit(nbits);//(0x1e0fffff);
...
const string strMessageMagic = "Dealtoken Signed Message:\n";
...
{
    return 0.00001 * COIN;//<zxb>定义pow奖励
...
int64 GetProofOfStakeReward(int64 nCoinAge,int64 balance)//<zxb>
{
    ...
    float rate = 50*pow(0.72556856,year);//zxb
    printf("rate ===== %f\n", rate);
...
void ThreadScriptCheck() {
    RenameThread("Dealtoken-scriptch");
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in Dealtoken
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    printf("pindexNew->nStakeModifierChecksum :%d\n",pindexNew->nStakeModifierChecksum );
    printf("pindexNew->nHeight :%d\n",pindexNew->nHeight );
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsLimit =%d,nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Dealtoken", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnProofOfWorkLimit.GetCompact(),bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    // printf("Initializing databases111 fReindex==%d...\n",fReindex);
    // printf("11111 uint256(0)==%d...\n",uint256(0));
    // printf("22222 uint256(0)==%s\n", uint256(0).ToString().c_str());
    //  CBigNum a(~uint256(0));
    // printf("333333 uint256(0)==%s\n", a.ToString().c_str());
    // a = a >> 32;
    // printf("44444 uint256(0)==%s\n", a.ToString().c_str());
    // printf("55555 uint256(0)==0x%08x\n", a.GetCompact());
...
// ~uint256(0) >> 32
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 20/3/2018 dealtoken genesis generated.";// "Matonis 07-AUG-2012 Parallel Currencies And The Roadmap To Monetary Freedom";
        CTransaction txNew;
        txNew.nTime = 1521475309;//
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1521475309;// 1345084287;
...
/////////////////////////////////zxb11111111111111111111111
 ...
 printf("block.nNonce 11111111111111 zxb:%d \n" ,block.nNonce);
        ...
        assert(block.hashMerkleRoot == uint256("8a6b529b2f2b94632916911a15b1dcfb551b137f18ce84e0e07522c42f822e0fXX"));
...
string GetWarnings(string strFor)
{
    ...
    //    strStatusBar = _("This is a pre-release test build - use at your own risk - do not use for mining or merchant applications");//JERRY
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    {
        printf("attemp to find a coinstake\n" );
    ...
    // Collect memory pool transactions into the block
     printf("Collect memory pool transactions into the block\n" );
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
        {
          ...
          printf("POW pindexPrev.nHeight===%d\n", pindexPrev->nHeight);
           ...
           {
                pblock->vtx[0].vout[0].nValue = 20000000000 * COIN;//首块产量<zxb>
           }
           else
           {
		
             pblock->vtx[0].vout[0].nValue = reward;//zxb
           }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("DealtokenMiner : proof-of-work not meeting target");

    //// debug print
    printf("DealtokenMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DealtokenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DealtokenMiner : ProcessBlock, block not accepted");
    }
...
#ifdef TESTING
void DealtokenMiner(CWallet *pwallet, bool fProofOfStake, bool fGenerateSingleBlock)
#else
void DealtokenMiner(CWallet *pwallet, bool fProofOfStake)
#endif
{
    ...
    try { loop {
        ...
        CBlockIndex* pindexPrev = pindexBest;
        printf("CreateNewBlock CreateNewBlock proof-of-%s \n",fProofOfStake? "stake" : "work" );
        ...
        printf("Running DealtokenMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        printf("start pow minner nBits=%d\n",pblock->nBits );
        uint256 hashTarget = CBigNum().SetCompact(pblock->nBits).getuint256();
        uint256 hashbuf[2];
        ...
        loop
        {
            ...
            if (nNonceFound != (unsigned int) -1)
            {
                ...
                if (hash <= hashTarget)
                {
                    printf("minner success nNonceFound==%d\n",nNonceFound );
                    // Found a solution
                    pblock->nNonce = ByteReverse(nNonceFound);
                    assert(hash == pblock->GetHash());
                    if (!pblock->SignBlock(*pwalletMain))
                    {
                         printf("minner success strMintMessage==%s\n",strMintMessage.c_str() );
                        strMintWarning = strMintMessage;
                        break;
                    }
                    printf("minner success checkWork\n" );
                    strMintWarning = "";
                    SetThreadPriority(THREAD_PRIORITY_NORMAL);
                    CheckWork(pblock, *pwalletMain, reservekey);
                    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        if(!fProofOfStake) printf("DealtokenMiner terminated\n");
        throw;
    }
}

void GenerateDealtokens(bool fGenerate, CWallet* pwallet)
{
...
#ifdef TESTING
        minerThreads->create_thread(boost::bind(&DealtokenMiner, pwallet, false, false));
#else
        minerThreads->create_thread(boost::bind(&DealtokenMiner, pwallet, false));
#endif
}
